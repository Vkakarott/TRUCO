...
typedef struct {
    Carta rodada[NUMERO_PLAYERS];
    char jogadorVencedor[CHAR_SIZE];
    int maiorCarta;
} Mesa ;

typedef struct {
    char jogador1[CHAR_SIZE];
    char jogador2[CHAR_SIZE];
    int pontos;
} Dupla ;
...
Dupla dupla1, dupla2;
Jogador jogadores[NUMERO_PLAYERS];
...
void truco(int client_socket,int rodadaAtual, int *pontosMesa, int valorTruco){
    ...
    char player_choice[CHAR_SIZE];
    switch (valorTruco){
    case 1:
        printf("TRUCO!\nO jogador %s pediu truco!\n1 - Aceitar\n2 - Pedir seis\n3 - Correr\n", jogadores[client_socket].nome);
        adversario = (client_socket < NUMERO_PLAYERS - 1) ? client_socket+1 : client_socket - 3;
        strcpy(message, "0 TRUCO!");
        for (i = 0; i < NUMERO_PLAYERS; i++) sendToClient(jogadores[i].clt_skt, message);
...
void jogada(int client_socket, int rodadaAtual, int pontosMesa) {
    ...
    if(player_choice_int == 0) truco(client_socket, rodadaAtual, &pontosMesa, 1);
        ...
        rodadas[rodadaAtual].rodada[client_socket] = jogadores[client_socket].mao[player_choice_int-1];
        reorganizarMao(jogadores[client_socket], player_choice_int-1);
    }
...
void verificarVencedor(Mesa mesa, int pontosMesa, int rodadaAtual){
    ...
    int advrs2 = (jogadorVencedor < NUMERO_PLAYERS - 1) ? jogadorVencedor+1 : jogadorVencedor - 3;
...
void rodada(int jogadas, int rodadaAtual, int jogadorAtual, int pontosMesa){
    ...
    rodada(jogadas+1, rodadaAtual, (jogadorAtual < NUMERO_PLAYERS -1) ? jogadorAtual+1 : jogadorAtual - 3, pontosMesa);
}
